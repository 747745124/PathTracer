...
struct MaterialIO {
  ...
  Flt shininess;    /* Shininess: 0.0 - 1.0.  Must be scaled (multiply by 128)
                       before use as a Phong    cosine exponent (q in our equation).
                     */
...
struct PolySetIO {
   ...
   */
  int rowSize;     /* Number of primitives per row		*/
  int numPolys;    /* Number of polygons		   	*/
  PolygonIO *poly; /* Polygonal array			   	*/
};
