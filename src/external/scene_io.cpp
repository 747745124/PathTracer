...
void writeSceneBinary(SceneIO *scene, const char *filename) {
  ...
  fwrite(&Test_int, sizeof(int), 1, fp);
...
static LightIO *read_lightsB(FILE *fp) {
  ...
  fread(&num_lights, sizeof(int), 1, fp);
  if (num_lights == 0)
    return lights;
...
static ObjIO *read_objectsB(FILE *fp) {
  ...
  fread(&num_objects, sizeof(int), 1, fp);
  if (num_objects == 0)
    return objects;
...
static void write_poly_setA(ObjIO *obj, FILE *fp) {
  ...
  case PolySetType::POLYSET_TRI_MESH:
    ...
    break;
  case PolySetType::POLYSET_FACE_SET:
    fprintf(fp, "  type POLYSET_FACE_SET\n");
    break;
  case PolySetType::POLYSET_QUAD_MESH:
...
static void read_poly_setB(ObjIO *obj, FILE *fp) {
  ...
  fread(&pset->rowSize, sizeof(int), 1, fp);
  fread(&pset->numPolys, sizeof(int), 1, fp);
  pset->poly = (PolygonIO *)calloc(pset->numPolys, sizeof(PolygonIO));
  poly = pset->poly;
