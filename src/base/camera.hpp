...
public:
    ...
    float focalDist;
    float aperture;
    PerspectiveCamera(float fov, float aspect, float focalDist,float aperture, gl::vec3 up, gl::vec3 front, gl::vec3 position);
    PerspectiveCamera(const CameraIO *cameraIO, float aspect = 1.0f,float aperture = 2.0f);
