...
static std::array<int, 512U> perm = {
...
static int get_next(int num, int size) { return (num + 1) % size; };
...
static float fractal_perlin_2D(gl::vec2 uv, int octaves) {
  ...
  for (int i = 0; i < octaves; i++) {
    total += amp * perlin_noise_2D(uv * freq);
    amp *= 0.5f;
    freq *= 2.f;
  }

  return std::clamp(fabs(total), 0.f, 1.f);
}
